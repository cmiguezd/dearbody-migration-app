...
authRouter.get("/install", (req, res) => {
  ...
  if (!shop.endsWith(".myshopify.com")) {
    return res.status(400).send("A valid Shopify shop is required.");
  }
  ...
  const url =
    ...
    encodeURIComponent(process.env.SHOPIFY_API_KEY || "") +
    "&scope=" +
...
authRouter.get("/callback", async (req, res) => {
  ...
  if (!response.ok) {
    return res.status(502).send("Shopify OAuth exchange failed.");
  }
