...
export async function executeMigration(plan: MigrationPlan): Promise<{ reports: Report[] }> {
  ...
  if (plan.selection.menus) { const result = await from.query<{ menus: { nodes: Array<{ title: string; handle: string; items: Array<{ title: string; url: string; type: string; items: Array<{ title: string; url: string; type: string }> }> }> } }>(menusQuery); let count = 0; for (const menu of result.menus.nodes) { if (!plan.dryRun) { const created = await to.query<{ menuCreate: { userErrors: Array<{ message: string }> } }>(createMenu, { title: menu.title, handle: menu.handle, items: menu.items.map((item) => ({ title: item.title, url: item.url, type: item.type, items: item.items.map((child) => ({ title: child.title, url: child.url, type: child.type })) })) }); if (created.menuCreate.userErrors.length && !created.menuCreate.userErrors.some((e) => /already exists|taken/i.test(e.message))) throw new Error(created.menuCreate.userErrors.map((e) => e.message).join("; ")); } count++; } reports.push({ module: "menus", status: plan.dryRun ? "skipped" : "completed", count }); }
  if (plan.selection.files) { const result = await from.query<{ files: { nodes: Array<{ alt: string | null; contentType: string | null; url: string }> } }>(filesQuery); if (plan.dryRun) reports.push({ module: "files", status: "skipped", count: result.files.nodes.length }); else { const created = await to.query<{ fileCreate: { files: Array<{ id: string }>; userErrors: Array<{ message: string }> } }>(createFiles, { files: result.files.nodes.map((file) => ({ originalSource: file.url, alt: file.alt, contentType: file.contentType })) }); if (created.fileCreate.userErrors.length) throw new Error(created.fileCreate.userErrors.map((e) => e.message).join("; ")); reports.push({ module: "files", status: "completed", count: created.fileCreate.files.length }); } }
  if (plan.selection.collections) reports.push({ module: "collections", status: "failed", message: "Las colecciones se mantienen como módulo opcional y requieren mapeo de reglas." });
  return { reports };
}
